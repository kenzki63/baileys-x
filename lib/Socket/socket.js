...
const makeSocket = (config) => {
    ...
    let connectionValidationError;
    const connectionValidationWaiters = [];
    const markConnectionValidated = () => {
        connectionValidated = true;
        while (connectionValidationWaiters.length) {
            connectionValidationWaiters.shift().resolve();
        }
    };
    const markConnectionValidationFailed = (err) => {
        if (connectionValidated || connectionValidationError) {
            return;
        }
        connectionValidationError = err || new boom_1.Boom('Connection Closed', { statusCode: Types_1.DisconnectReason.connectionClosed });
        while (connectionValidationWaiters.length) {
            connectionValidationWaiters.shift().reject(connectionValidationError);
        }
    };
    const waitForConnectionValidated = () => {
        if (connectionValidated) {
            return Promise.resolve();
        }
        if (connectionValidationError) {
            return Promise.reject(connectionValidationError);
        }
        return new Promise((resolve, reject) => connectionValidationWaiters.push({ resolve, reject }));
    };
    ...
    const end = (error) => {
        ...
        closed = true;
        markConnectionValidationFailed(error);
    ...
    const requestPairingCode = async (phoneNumber, pairKey, Bytesrandom = 32) => {

    if (pairKey) {
        const customPairingCode = String(pairKey).toUpperCase();
    ...
    ws.on('open', async () => {
        try {
            ...
            markConnectionValidationFailed(err);
            end(err);
        }
    });
